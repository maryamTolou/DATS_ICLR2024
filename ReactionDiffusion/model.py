...
from ReactionDiffusion import Equation, Test, Load_ground_truth, Generate_train_data
...
class PINN():
    def __init__(self, network, device, model_type, hypernet, hparams, json_path):
        ...
        for i in range(0, 3):
            for j in range(0, 3):
                self.params.append((round(2.0 * i + 1.0, 2), round(2.0 * j + 1.0, 2)))
        ...
        if (self.residual_sampling == False or self.sampling_type == 'uniform') and self.hparams.residual_sampler[
            ...
            self.t_train_f, self.x_train_f, _, _, _, _, _, _, _ = Generate_train_data(self.num_train, self.num_ic, self.num_bc, self.ic_func, self.device)
            for i, task in enumerate(self.params):
    ...
    def net_f(self, t, x, latent):
        ...
        f = Equation(u, t, x, self.a1, self.a2)
        return f
    ...
    def RAD(self, task):
        ...
        ps = {}
        t_test, x_test, _, _, _, _, _, _, _ = Generate_train_data(20000, self.num_ic, self.num_bc, self.ic_func, self.device)
        if self.model_type == 'hyper-pinn':
            net_weight, net_bias = self.hypernet(self.a1, self.a2)
        ...
        f_pred = Equation(u_pred, t_test, x_test, self.a1, self.a2)
        loss_f = f_pred
            ...
            for task in self.params:
                ...
                if num_train > 0:
                    if self.hparams.residual_sampler['type'] == 'rad':
                        ...
                        t_train_f, x_train_f, _, _, _, _, _, _, _ = Generate_train_data(self.num_train, self.num_ic, self.num_bc,
    ...
    def loss_func(self):
        ...
        if self.sampling_type == "uniform":
            for i, task in enumerate(self.params):
                ...
                u_bc1_x = torch.autograd.grad(u_bc1_pred, self.x_bc1_train, grad_outputs=torch.ones_like(u_bc1_pred),
                                              retain_graph=True, create_graph=True)[0]
                u_bc2_x = torch.autograd.grad(u_bc2_pred, self.x_bc2_train, grad_outputs=torch.ones_like(u_bc2_pred),
                                              retain_graph=True, create_graph=True)[0]
                loss_b = torch.mean((u_bc1_x - u_bc2_x) ** 2)
                if self.model_type == 'hyper-pinn':
                    scaled_loss = loss_u + self.f_scale*(loss_b + loss_f)
                elif self.model_type == 'mad-pinn':
                    loss_reg = torch.mean(torch.square(self.latent_vector))
                    scaled_loss = loss_u + self.f_scale*(loss_b + loss_f) + self.latent_reg * loss_reg
                else:
                    NotImplementedError
    ...
    def task_sampler(self):
        ...
        for i, task in enumerate(self.params):
            ...
            if self.iter % self.prob_period == 0:
                ...
                u_bc1_x = torch.autograd.grad(u_bc1_pred, self.x_bc1_val, grad_outputs=torch.ones_like(u_bc1_pred),
                                              retain_graph=True, create_graph=True)[0]
                u_bc2_x = torch.autograd.grad(u_bc2_pred, self.x_bc2_val, grad_outputs=torch.ones_like(u_bc2_pred),
                                              retain_graph=True, create_graph=True)[0]
                loss_b = torch.mean((u_bc1_x - u_bc2_x) ** 2)
                if self.model_type == 'hyper-pinn':
                    scaled_loss = loss_u + self.f_scale * (loss_b + loss_f)
    ...
    def self_pace_loss(self):
        ...
        for i, task in enumerate(self.params):
            ...
            loss_b = torch.mean((u_bc1_x - u_bc2_x) ** 2)
            if self.model_type == 'hyper-pinn':
                loss = (loss_u + self.f_scale * (loss_b + loss_f))
            elif self.model_type == 'mad-pinn':
                loss_reg = torch.mean(torch.square(self.latent_vector))
                loss = (loss_u + self.f_scale * (loss_b + loss_f) + self.latent_reg * loss_reg)
            else:
                NotImplementedError
    ...
    def test(self, it):
        ...
        for i, task in enumerate(self.params):
            a1, a2 = task
            self.a1, self.a2 = torch.tensor([a1]), torch.tensor([a2])
            self.t_test, self.x_test, self.u_test = Load_ground_truth(a1, a2, self.ic_func)
            if self.model_type == 'hyper-pinn':
                ...
                latent = self.network.latent_vector[self.latent_id[task]]
            l2_loss, mse_loss = Test(self.t_test, self.x_test, self.u_test, self.net_u, it, self.output_path, str(a1) + '-' + str(a2), self.device, latent)
