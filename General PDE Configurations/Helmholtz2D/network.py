...
class Base(nn.Module):
    def __init__(self, hparams):
        ...
        if self.model_type == 'mad-pinn':
            ...
            latent_init = torch.randn([hparams.sampler['n_way'], self.latent_size]) / torch.sqrt(torch.tensor(self.latent_size, dtype=torch.float))
