...
def plot_val_over_epoch(output_path, plot_name, epochs, tasks, task_probs, colors, x_title, y_title, plot_title):
...
def plot_loss_convergence(output_path, nus, colors, plot_name, loss_dict, thresholds):
    epochs = range(1, len(list(loss_dict.values())[0]) + 1)
    # num_tasks = len(loss_dict)
...
def find_closest(z_map, z_i, nu):
    tasks = np.array(list(z_map.keys()))
    task_diffs = np.abs(np.log(tasks) - np.log(nu))
    closest_task_idx = np.argmin(task_diffs)
    closest_task_id = z_map[tasks[closest_task_idx]]
    closest_z_i = z_i[closest_task_id]
    
    return closest_z_i, closest_task_id