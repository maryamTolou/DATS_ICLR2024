...
class PINN():
    def __init__(self, network, device, model_type, hypernet, hparams, json_path):
        ...
        for i in range(3):
            for j in range(3):
                param1 = (0.5 * i + 1)
                param2 = (1.0 * j + 1)
                self.params.append((param1, param2))
        self.nu = 0.0
        self.beta = 1
    ...
    def RAD(self, task):
        ...
        f_pred = Equation(u_pred, t_test, x_test, self.beta)
        loss_f = f_pred
    ...
    def generate_epoch_dataset(self, iteration):
        if self.residual_sampling is True and self.sampling_type != 'uniform' and self.iter % self.sampler['period'] == 0:
            if self.hparams.residual_sampler['type'] == 'rad':
                for task in self.params:
                    a1, a2 = task
                    self.ic_func = lambda x: a1 * np.sin(a2 * x)
    ...
    def loss_func(self):
        ...
        if self.sampling_type == "uniform":
            for i, task in enumerate(self.params):
                ...
                self.ic_func = lambda x: a1 * np.sin(a2 * x)
                self.a1, self.a2 = torch.tensor([a1]).to(self.device), torch.tensor([a2]).to(self.device)
                _, _, self.t_train, self.x_train, self.u_train, self.t_bc1_train, self.x_bc1_train, self.t_bc2_train, self.x_bc2_train = Generate_train_data(
                    self.num_train, self.num_ic, self.num_bc, self.ic_func, self.device)
                if self.model_type == 'hyper-pinn':
                    net_weight, net_bias = self.hypernet(self.a1)
                    self.network.set_params(net_weight, net_bias)
    ...
    def task_sampler(self):
        ...
        for i, task in enumerate(self.params):
            ...
            _, _, self.t_train, self.x_train, self.u_train, self.t_bc1_train, self.x_bc1_train, self.t_bc2_train, self.x_bc2_train = Generate_train_data(
                self.num_train, self.num_ic, self.num_bc, self.ic_func, self.device)
            _, _, self.t_val, self.x_val, self.u_val, self.t_bc1_val, self.x_bc1_val, self.t_bc2_val, self.x_bc2_val = Generate_train_data(
                self.num_val_res, self.num_ic_val, self.num_bc_val, self.ic_func, self.device)
            if self.model_type == 'hyper-pinn':
                net_weight, net_bias = self.hypernet(self.a1)
                self.network.set_params(net_weight, net_bias)
    ...
    def test(self, it):
        ...
        for i, task in enumerate(self.params):
            ...
            self.t_test, self.x_test, self.u_test = Load_ground_truth(self.nu, self.beta, self.ic_func)
            if self.model_type == 'hyper-pinn':
